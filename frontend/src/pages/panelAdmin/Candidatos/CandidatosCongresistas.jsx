...
export default function CandidatosCongresistas() {
  ...
  // Cargar congresistas desde la API
  useEffect(() => {
    const cargarCandidatos = async () => {
      try {
        setLoading(true);
        console.log("Cargando Congresistas desde:", API_ENDPOINTS.CANDIDATOS.CONGRESISTAS);
        
        const congresistas = await fetchAPI(API_ENDPOINTS.CANDIDATOS.CONGRESISTAS);
        console.log(" Datos recibidos de /congresistas:", congresistas);
        
        const candidatosBackend = Array.isArray(congresistas) ? congresistas : [];
        ...
        console.log(` Se cargaron ${candidatosConFotos.length} congresistas`);
      } catch (error) {
  ...
  const handleCrear = async (nuevoCandidato) => {
    try {
      console.log("=== Creando congresista ===", nuevoCandidato);
      const candidatoCreado = await crearCandidatoEnAPI(nuevoCandidato);
      console.log("=== Congresista creado ===", candidatoCreado);
      ...
      const cargarCandidatos = async () => {
        try {
          ...
          console.error("Error al recargar congresistas:", error);
        } finally {
          setLoading(false);
        }
      };
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600">Cargando congresistas...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Congresistas</h1>
            <p className="text-sm text-gray-600">Administra la información de los candidatos al Congreso.</p>
          </div>
        </div>
        <button
